...
void solve() {
  ...
  while (t--)
  {
   ...
   vector<vector<ll>> values(n,vector<ll>());
   vector<ll> remneb(n,0);
   vector<ll> maxkval(n,0);
   ...
   queue<ll> nodewithoneneb;

   for(ll i=0;i<n;i++){
       if(remneb[i] == 1){
           nodewithoneneb.push(i);
           maxkval[i] = 1;
       }
   }

   while(!nodewithoneneb.empty()){
       ll curnode = nodewithoneneb.front();
       nodewithoneneb.pop();

       for(ll child:values[curnode]){
           if(remneb[child] != 1){
               remneb[child]--;

               if(remneb[child] == 1){
                   maxkval[child] = maxkval[curnode] + 1;
                   nodewithoneneb.push(child);
               }
           }
       }
   }
