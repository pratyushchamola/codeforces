...
//    ll tempn = n;
   for(ll i=2;(i*i) <= n;i++){
       if(n%i == 0){
           ...
           if(cnt > 0)cntval.push_back({cnt,i});
       }
   }

   if(n > 1)cntval.push_back({1,n});

   sort(cntval.rbegin(),cntval.rend());

   vector<ll> ans(cntval[0].first,cntval[0].second);

   for(ll i=1;i<cntval.size();i++){
       for(ll j=0;j<cntval[i].first;j++){
           ans.back() *= cntval[i].second;
       }
   }
