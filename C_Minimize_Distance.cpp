...
void solve() {
  ...
  while (t--)
  {
   ...
   for(ll i=0;i<n;i++){
       ...
       if(curr >= 0)forpos.push_back(curr);
       else{
           curr = abs(curr);
           forneg.push_back(curr);
       }
   }
   ...
   ll posans = 0;
   ll negans = 0;
   for(ll i=forpos.size()-1;i>=0;i -= k){
       posans += forpos[i];
   } 

   for(ll i=forneg.size()-1;i>=0;i -= k){
       negans += forneg[i];
   }

   ll ans = 2*(posans + negans);

   if(forpos.size() > 0 && forneg.size() > 0){
       ans -= max(forpos[forpos.size()-1], forneg[forneg.size()-1]);
   }else if(forpos.size() > 0){
       ans -= forpos[forpos.size()-1];
   }else if(forneg.size() > 0){
       ans -= forneg[forneg.size()-1];
   }
