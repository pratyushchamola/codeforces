...
bool check(ll curans,vector<ll>& arr,ll sz){
    vector<ll> curarr;

    for(ll i=0;i<sz;i++)curarr.push_back(arr[i]);

    for(ll i=sz-1;i>=2;i--){
        if(curarr[i] < curans)return false;
        ll d = min(arr[i],curarr[i]-curans)/3;

        curarr[i-1] += d;
        curarr[i-2] += 2*d;
    }

    return (curarr[0] >= curans) && (curarr[1] >= curans);
}
...
//   for (int i = 1;i<=2e5;i++)fact[i] = (i * fact[i - 1])%MOD;
  ...
  while (t--)
  {
   ...
   ll l = 0, r = *max_element(values.begin(),values.end());

   while(l < r){
       ll mid = l + (r - l + 1)/2;

       if(check(mid,values,n))l = mid;
       else r = mid - 1;
   } 
