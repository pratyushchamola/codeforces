...
void solve() {
  ...
  ll sum = 0;
  for(ll i=0;i<n;i++){
      cin >> values[i].second >> values[i].first;
      sum += values[i].first;
  }
  ...
  while(i < j){
      if(values[i].first <= ex){
          ans += values[i].second;
          ex += values[i].second;
          i++;
      }else if(values[j].first <= ex || values[j].second == 0){
          ans += values[j].second;
          ex += values[j].second;
          j--;
      }else{
          ll req = values[i].first - ex;
          ll pre = min(req,values[j].second);
