...
vector<vector<ll>> values(501,vector<ll>(501,0));
ll n;
bool findans(ll curval,ll currow,ll curcol,ll cnt){
    if(cnt >= curval)return true;

    if(currow >= n || curcol < 0)return false;

    // if(curcol >= n)return;

    values[currow][curcol] = curval;
    if((curcol-1) >= 0 && values[currow][curcol-1] == 0){
        if(findans(curval,currow,curcol-1,cnt + 1))return true;
    }

    findans(curval,currow+1,curcol,cnt+1);
...
//   ll n;
  ...
  vector<ll> diagonals(n);

  for(ll i=0;i<n;i++)cin >> diagonals[i];

  for(ll i=0;i<n;i++){
      findans(diagonals[i],i,i,0);
  }
