...
void dfs(vector<vector<ll>>& graph,ll curcol,ll currow){
    if(currow < 1 || currow > n ){
        ans.push_back(curcol);
        return;
    }

    ll curval = graph[currow][curcol];

    graph[currow][curcol] = 2;

    if(curval ==1)dfs(graph,curcol+1,currow);
    if(curval == 2)dfs(graph,curcol,currow+1);
    if(curval == 3)dfs(graph,curcol-1,currow);
}
void solve() {
    // ll n,m,k;
