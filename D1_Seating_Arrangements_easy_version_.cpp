...
//   for (int i = 1;i<=2e5;i++)fact[i] = (i * fact[i - 1])%MOD;
  ...
  while (t--)
  {
   ...
   for(ll i=0;i< n*m;i++)values[i].second = -values[i].second;

   ll ans = 0;

   for(ll i=0;i<n;i++){
       sort(values.begin() + m*i,values.begin() + (m*i + m));

       for(ll j=0;j<m;j++){
           for(ll k=0;k<j;k++){
               if(values[i*m + k].second > values[i*m + j].second)ans++;
           }
       }
   }

//    for(ll i=0;i<m;i++)cout << values[i].second << " ";
