...
vector<vector<ll>> visited(1004,vector<ll>(1004,0));
ll n,m;
ll cnt = 0;

void dfs(vector<vector<ll>>& arr,ll row,ll col){
    if(row < 0 || row >= n || col < 0 || col >= m || visited[row][col]==1)return;
    ...
    // if(row+1 < n && visited[row+1][col] == 0){
        ll checknorth = arr[row][col] & (1<<3);
        ll checksouth = arr[row][col] & (1<<1);

    //     if(checknorth == 0 && checksouth == 0 )dfs(arr,row+1,col);
    // }

    // if(col+1 < m && visited[row][col+1] == 0){
        ll checkwest = arr[row][col] & (1<<0);
        ll checkeast = arr[row][col] & (1<<2);

    //     if(checkwest == 0 && checkeast == 0)dfs(arr,row,col+1);
    // }
    
    if(checknorth == 0)dfs(arr, row-1,col);
    if(checkeast == 0)dfs(arr, row,col+1);
    if(checksouth == 0)dfs(arr, row+1,col);
    if(checkwest == 0)dfs(arr, row,col-1);
