...
void solve() {
  ...
  for(ll i=n-1;i>0;i--){
      ...
      string secstr = s.substr(0,i);

      while(curstr.length() < k)curstr += s.substr(0,i+1);

      curstr = curstr.substr(0,k);

    //   ans = min(ans,curstr);

    if(curstr < ans)ans = curstr;

      while(secstr.length() < k)secstr += s.substr(0,i);

      secstr = secstr.substr(0,k);

    //   ans = min(ans,secstr);

    if(secstr < ans)ans = secstr;
  }
