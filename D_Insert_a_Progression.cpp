...
void solve() {
  ...
  while (t--)
  {
   ...
   for(int i=0;i<2;i++){
       ll mn = abs(a[0] - 1);
       ans = min(ans,cur + abs(x - a[0]) + (x-1));

       for(int j=0;j<n-1;j++){
           ans = min(ans,cur - abs(a[j] - a[j+1]) + abs(a[j] - x) + abs(a[j+1] - x) + mn);
           ans = min(ans,cur - abs(a[j] - a[j+1]) + abs(a[j] - 1) + abs(a[j+1] - x) + (x-1));
           mn = min(mn, abs(a[j] - 1) + abs(a[j+1] - 1) - abs(a[j] - a[j+1]));
       }
