...
void solve() {
  ...
  while (t--)
  {
   ...
   for(ll i=0;i<n;i++){
       ans += upper_bound(values.begin(),values.end(),r - values[i]) - values.begin();
       ans -= lower_bound(values.begin(),values.end(),l - values[i]) - values.begin();

       if(l <= 2*values[i] && 2*values[i]<=r)ans--;
   } 
