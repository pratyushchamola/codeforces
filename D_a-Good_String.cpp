...
ll findans(string s,char ch){
    ...
    if(s.size() == 1){
       ...
       if(cnt2 == (s.size()>>1)){
        ...
        ll temp1 = ((s.size()>>1) - cnt1) + findans(s.substr(s.size()>>1),ch+1);
        ll temp2 = ((s.size()>>1) - cnt2) + findans(s.substr(0,s.size()>>1),ch+1);
