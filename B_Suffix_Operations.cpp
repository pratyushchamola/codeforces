...
void solve() {
  ...
  while (t--)
  {
   ...
   for(ll i=1;i<n;i++)ans += abs(values[i-1] - values[i]);

   ll curmx = max(abs(values[0]-values[1]),abs(values[n-1]-values[n-2]));

   for(ll i=1;i<n-1;i++){
       curmx = max(curmx,abs(values[i] - values[i-1]) + abs(values[i] - values[i+1]) - abs(values[i-1] - values[i+1]));
   } 
