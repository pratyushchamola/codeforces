...
// 2*win - winningstreaks
    ll totwins = 0, totloss = 0, winningstreak = 0;
   for(ll i=0;i<n;i++){
       if(s[i] == 'W'){
           totwins++;
           if(i == 0 || s[i-1] == 'L')winningstreak++;
       }else{
           totloss++;
           if(i == 0 || s[i-1] == 'W')llen.push_back(0);
           llen.back()++;
       }
   }

   if(s[0] == 'L')llen[0] = 1e8;
   if(s[n-1] == 'L')llen.back() = 1e8;
   ...
   cout << 2*totwins - winningstreak << endl;
