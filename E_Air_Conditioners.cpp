...
void solve() {
  ...
  while (t--)
  {
   ...
   vector<pair<ll,ll>> ans(n+1,{INT_MAX,INT_MAX});
   vector<pair<ll,ll>> acs(k);
   ...
   bool flag = false;
   ll m=0;
   ll prevind = 0, prevtemp = 0;
   for(ll i=1;i<=n;i++){
       if((m >= acs.size()) || (acs[m].first > i)){
           if(flag==false)continue;

           ans[i].first = (prevtemp + abs(prevind - i)); 
       }else{
           flag = true;

           ans[i].first = (prevtemp == 0) ? acs[m].second : min(acs[m].second, prevtemp + abs(prevind - i));

           prevtemp = (prevtemp == 0) ? acs[m].second : (acs[m].second <= (prevtemp + abs(prevind - i)) ? acs[m].second : prevtemp);
           prevind = (prevind == 0) ? acs[m].first : (acs[m].second <= (prevtemp + abs(prevind - i)) ? acs[m].first : prevind);
           m++;
       }
   }
   ...
   m = k-1;
   for(ll i=n;i>0;i--){
       if((m < 0) || (acs[m].first < i)){
           if(flag==false)continue;

           ans[i].second = (prevtemp + abs(prevind - i)); 
       }else{
           flag = true;

           ans[i].second = (prevtemp == 0) ? acs[m].second : min(acs[m].second, prevtemp + abs(prevind - i));
