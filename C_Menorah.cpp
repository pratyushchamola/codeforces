...
ll n;
ll oper(string a,string b){
    ll cntonezero = 0, cntzeroone = 0;

    for(ll i=0;i<n;i++){
        cntonezero += (a[i]=='1' && b[i]=='0');
        cntzeroone += (a[i]=='0' && b[i]=='1');
    }

    if(cntonezero != cntzeroone)return INT_MAX;

    return cntonezero + cntzeroone;
}

ll changeit(string a,string b,char ch){
    ll ind = -1;
...
//    ll n;
   ...
   ll ans = oper(a,b);
   ans = min(ans,1 + changeit(a,b,'0'));
   ans = min(ans,1 + changeit(a,b,'1')); 
