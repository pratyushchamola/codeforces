...
void solve() {
  ...
  while (t--)
  {
   ...
   for(ll i=0;i<n;i++){
       ...
       total += curr;
       values[uni[i]].push_back(curr);
   }

   for(ll i=1;i<=n;i++)sort(values[i].begin(),values[i].end(), greater<ll>());

   vector<vector<ll>> preans(n+1);

   for(ll i=1;i<=n;i++){
       for(int child:values[i]){
           if(preans[i].size()==0)preans[i].push_back(child);
           else preans[i].push_back(preans[i].back() + child);
       }
   }
   ...
   for(ll i=1;i<=n;i++){
       for(ll k=1;k<=(int)values[i].size();k++){
           ans[k-1] += preans[i][(values[i].size()/k*k)-1];
       }
   }
