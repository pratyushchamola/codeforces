...
vector<ll> a[300001];
vector<ll> ch(300001,0), dp(300001,0);

void dfs(int curnode,int par){
    ch[curnode] = 1, dp[curnode] = 0;
    int s = 0;
    for(auto child:a[curnode]){
    ...
    for(auto child:a[curnode]){
        if(child == par)continue;
        dp[curnode] = max(dp[curnode], s - dp[child] + ch[child] - 1);
    }
