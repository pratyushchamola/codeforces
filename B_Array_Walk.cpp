...
void solve() {
  ...
  while (t--)
  {
   ...
   ll curans = presum[k+1];

   ll cnt = 1;
   while(cnt<=z){
       ll curmoves = 0;

       while(!s.empty() and (s.top().second > (k - (2*cnt - 1))))s.pop();

       if(s.empty())break;

       ll curind = s.top().second;
       ll tempans = presum[curind+1];
       curmoves += curind;
       bool flag = true;
       ll temp = 0;
       while(temp<cnt){
           ...
           else tempans += a[curind];
           flag = !flag;
           curmoves++;
       }

       if(curmoves < k){
           if(flag)tempans += (presum[curind+(k-curmoves)+1] - presum[curind+1]);
           else tempans += (presum[curind-1 + (k-curmoves)+1] - presum[curind]);
       }
