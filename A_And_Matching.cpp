...
void solve() {
  ...
  while (t--)
  {
   ...
   if(k < (n-1)){
       answer.push_back({k,n-1});
       visited[n-1] = visited[k] = true;

       if(k != 0){
           answer.push_back({0,n-1-k});
           visited[0] = true;
           visited[n-1-k] = true;
       }
       ...
       for(ll i=0;i<n;i++){
           ...
           answer.push_back({i,n-1-i});

           visited[i] = visited[n-1-i] = true;
       }
   }else{
       ...
       answer.push_back({n-2,n-1});
       answer.push_back({1,n-3});
       answer.push_back({0,2});

       visited[n-2] = visited[n-1] = visited[1] = visited[n-3] = visited[0] = visited[2] = true;
   ...
   for(ll i=0;i<answer.size();i++)cout << answer[i].first << " " <<  answer[i].second << endl;
