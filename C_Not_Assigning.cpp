...
const int maxlim = 1e5 + 1;
vector<pair<ll,ll>> adj[maxlim];
vector<ll> visited(maxlim,0);
   ...
   while(!q.empty()){
       ...
       for(auto child:adj[curnode]){
           if(visited[child.first] == 0){
               ans[child.second] = curans;
               q.push(child.first);
           }
       }
   }

   for(ll child:ans)cout << child << " ";
   cout << endl;
  }
}
