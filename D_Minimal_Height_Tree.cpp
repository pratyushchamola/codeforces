...
void solve() {
  ...
  while (t--)
  {
   ...
   while((q.size()>0) && (i<n)){
       ...
       while(((arr[curnode.first].size() == 0) || (arr[curnode.first].back() < a[i])) && (i<n)){
           arr[curnode.first].push_back(a[i]);
           q.push({a[i],curnode.second+1});
           level = max(level,(curnode.second+1));
           i++;
       }
   }
