...
void solve() {
  ...
  set<pair<ll,ll>> indbased, moneybased;
    ll ind = 1;
  while(q--){
      ll type;
      cin >> type;
      if(type==1){
        ...
        //   cout << "ind : " << ind << endl;
          indbased.insert({ind,estprice});
          moneybased.insert({-estprice,ind});
          ind++;
      }else if(type == 2){
          cout << (*indbased.begin()).first << " ";

          if(moneybased.find({-((*indbased.begin()).second),(*indbased.begin()).first}) != moneybased.end())moneybased.erase(moneybased.find({-((*indbased.begin()).second),(*indbased.begin()).first}));
          indbased.erase(indbased.begin());
      }else{
          cout << (*moneybased.begin()).second << " ";

          if(indbased.find({(*moneybased.begin()).second,-((*moneybased.begin()).first)}) != indbased.end())indbased.erase(indbased.find({(*moneybased.begin()).second,-((*moneybased.begin()).first)}));
          moneybased.erase(moneybased.begin());
      }
  }
}
