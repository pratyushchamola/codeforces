...
const ll maxlim = 2e5 + 10;
vector<ll> adj[maxlim];
ll lr[2][maxlim], dp[2][maxlim];
...
void dfs(ll node, ll par = -1){
    ...
    for(ll child:adj[node]){
        ...
        dp[0][node] += max((abs(lr[0][node] - lr[0][child]) + dp[0][child]), (abs(lr[0][node] - lr[1][child]) + dp[1][child]));
        dp[1][node] += max((abs(lr[1][node] - lr[0][child]) + dp[0][child]),(abs(lr[1][node] - lr[1][child]) + dp[1][child]));
    }
}
void solve() {
...
//    }
   ...
   for(ll i=1;i<=n;i++)cin >> lr[0][i] >> lr[1][i];
