...
ll n;
// const ll maxlim = 2e12 + 5;
ll maxlim;
// vector<vector<ll>> dp(n+1,vector<ll>(maxlim,-1));
ll findans(vector<ll>& arr,int curind,int curcnt,int cursum,vector<vector<ll>>& dp){
    if(curind >= arr.size())return curcnt;

    if(dp[curind][cursum] != -1)return dp[curind][cursum];

    if((arr[curind] + cursum) < 0)return dp[curind][cursum] = findans(arr,curind+1,curcnt,cursum,dp);
    else return dp[curind][cursum] = max(findans(arr,curind+1,curcnt+1,cursum + arr[curind],dp), findans(arr,curind+1,curcnt,cursum,dp));
}
...
//   ll n;
  ...
  for(ll i=0;i<n;i++){
      ...
      if(values[i] > 0)maxlim += values[i];
  }

  
  vector<vector<ll>> dp(n+1,vector<ll>(maxlim,-1));
  ll ans = findans(values,0,0,0,dp);
