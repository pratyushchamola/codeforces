...
void solve() {
  ...
  while (t--)
  {
   ...
   vector<ll> answer(n+1,0), values(n+1), giftreceiver(n+1,0), notsentgift;
   ...
   ll cnt = 0;
   for(ll i=1;i<=n;i++){
       if(checkgifted.find(values[i]) == checkgifted.end()){
           answer[i] = values[i];

           giftreceiver[values[i]] = i;
        //    giftsender[i] = values[i];
           cnt++;
           checkgifted.insert(values[i]);
       }else{
           notsentgift.push_back(i);
       }
   }

   stack<ll> notgiftedyet;

   for(ll i=1;i<=n;i++){
       if(giftreceiver[i] == 0)notgiftedyet.push(i);
   }

   if(notsentgift.size() == 1){
       ll notreceivedgift = find(giftreceiver.begin()+1,giftreceiver.end(),0) - giftreceiver.begin();

       if(notreceivedgift == notsentgift[0]){
           answer[giftreceiver[values[notreceivedgift]]] = notreceivedgift;
           answer[notreceivedgift] = values[notreceivedgift];
       }else{
           answer[notsentgift[0]] = notreceivedgift;
       }
   }else if(notsentgift.size() > 1){
       int last;

       for(ll i=1;i<=n;i++){
           if(answer[i] != 0)continue;

           ll cur = notgiftedyet.top();
           notgiftedyet.pop();

           if(cur == i){
               if(!notgiftedyet.empty()){
                   answer[i] = notgiftedyet.top();
                   notgiftedyet.pop();
                   notgiftedyet.push(cur);
                   last = i;
               }else{
                   answer[i] = cur;
