...
int bcand(vector<ll>& arr,int cur){
    ll ans = 10ll*INT_MAX;
    ll pos = -1;

    for(int i=0;i<arr.size();i++){
        if(abs(arr[i] - cur) < ans){
            ans = abs(arr[i] - cur);
            pos = i;
        }
    }
...
void solve() {
  ...
  while (t--)
  {
   ...
   vector<ll> zvals = {0,bcand(b,a[0]),n-1};
   vector<ll> nvals = {0,bcand(b,a[n-1]),n-1};

   ll ans = 10ll*INT_MAX;

   for(auto za:zvals){
       for(auto na:nvals){
           ll curans = abs(b[za] - a[0]) + abs(b[na] - a[n-1]);

           if(za>0 and na>0){
               curans += abs(b[0] - a[bcand(a,b[0])]);
           }

           if(za<(n-1) and na<(n-1)){
               curans += abs(b[n-1] - a[bcand(a,b[n-1])]);
           }
